...
class RivaGAN(object):
    ...
    def __init__(self, model="attention", data_dim=400):
        self.model = model
        ...
        if model == "attention":
            ...
            self.encoder = models.video.r3d_18(pretrained=False, progress=True).cuda()
            self.decoder = models.video.r3d_18(pretrained=False, progress=True).cuda()
        ...
        for epoch in range(1, epochs + 1):
            ...
            if use_critic or use_adversary:
                ...
                for frames in iterator:
                    ...
                    loss.backward()
                    G_opt.step()
            # self.encoder=self.encoder.Linear(64, 3, 7, 7)
            # self.encoder=self.encoder.Linear(64, 3, 7, 7)
            ...
            for frames in iterator:
                ...
                wm_frames = self.encoder(frames)
                
                wm_raw_data = self.decoder(noise(frames))
                wm_mjpeg_data = self.decoder(mjpeg(frames))
                # wm_raw_data = self.decoder(noise(wm_frames))
                # wm_mjpeg_data = self.decoder(mjpeg(wm_frames))
                # torch.reshape(wm_raw_data,(32,400))
